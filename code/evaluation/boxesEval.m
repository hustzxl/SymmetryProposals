...
% plot results
[M,T,K]=size(recall); fSiz={'FontSize',17}; f=o.show;
for type=1:2
  ...
  set(gca, 'FontSize', 17, 'LineWidth', 1.75);
  %title('Improved Example Figure');
  s={'nw','ne'}; 
  %[legh,objh,outh,outm] = legend(hs(1,:),o.names,'Location',s{type});
  %set(objh,'linewidth',9);
  if(isempty(o.fName)), continue; end; s={'Cnt','IoU'};
  %savefig([o.resDir '/plots/' s{type} '-' o.data.split '-' o.fName],'png');
  %saveas(gcf,[o.resDir '/plots/' s{type} '-' o.data.split '-' o.fName],'png');
  print([o.resDir '/plots/' s{type} '-' o.data.split '-' o.fName],'-depsc','-r300')
end
end
