...
*/
  ...
  if (0) {
	for (int i = 0;i< N-1;i++) {
		std::cout<<Z2[i]->node1<<'\t'<<Z2[i]->node2<<'\t'<<Z2[i]->dist<<std::endl;
	}
...
static cv::Rect rect_merge(cv::Rect& r1,cv::Rect& r2) {
	cv::Rect rect;
	rect.x = min(r1.x,r2.x);
	rect.y = min(r1.y,r2.y);
	int x2 = max(r1.x + r1.width,r2.x + r2.width);
	int y2 = max(r2.y + r2.height,r2.y + r2.height);
	rect.width = x2 - rect.x;
	rect.height = y2 - rect.y;
	return rect;
}
static double angle2(cv::Point p1,cv::Point p2) {
	double angle = 0;
	double theta = double(p1.y - p2.y) /(double)(p1.x - p2.x);
	return theta = abs( atan(theta) );
}
static double cal_angle(cv::Rect& r1,cv::Rect& r2){
	cv::Point p1_top(r1.x + r1.width/2,r1.y);
	cv::Point p1_bottom(r1.x + r1.width/2,r1.y + r1.height);
	cv::Point p2_top(r2.x + r2.width/2,r2.y);
	cv::Point p2_bottom(r2.x + r2.width/2,r2.y + r2.height);
	return min( angle2(p1_top,p2_top), angle2(p1_bottom,p2_bottom) );
}
#undef min
#undef max

template <bool sorted>
static void generate_dendrogram(t_float * const Z, cluster_result & Z2, const t_index N,std::vector<Region> &regions,std::vector<cv::Rect>&temp) {
  ...
  for (t_index i=0; i<N-1; i++) {
    ...
    if (sorted) {
	  ...
	  //std::cout<<regions[0].stroke_mean_<<std::endl;
	  
//	  std::cout<<"test"<<std::endl;
//	  std::cout<<regions[node1].bbox_.x<<'\t'<<regions[node1].bbox_.y<<'\t'<<regions[node1].bbox_.width<<'\t'<<regions[node1].bbox_.height<<std::endl;
//	  std::cout<<regions[node2].bbox_.x<<'\t'<<regions[node2].bbox_.y<<'\t'<<regions[node2].bbox_.width<<'\t'<<regions[node2].bbox_.height<<std::endl;
//	  std::cout<<rect.x<<'\t'<<rect.y<<'\t'<<rect.width<<'\t'<<rect.height<<std::endl;
	  
	  double angle = cal_angle(temp[node1],temp[node2]);
	  if (angle < THRESHOLD)
		nodes.Union(node1, node2);
		cv::Rect rect = rect_merge(regions[node1].bbox_,regions[node2].bbox_);
		temp.push_back(rect);
	}
...
static int linkage_vector(t_float *X, int N, int dim, t_float * Z, unsigned char method, unsigned char metric,std::vector<Region> &regions) {
  ...
  //fprintf(stderr, " generating dendogram\n");
    if (method!=METHOD_METR_SINGLE) {
	  ...
	  std::vector<cv::Rect>temp(2*(N-1));
	  for (int i = 0;i< N- 1;i++)
		  temp.push_back(regions[i].bbox_);
      generate_dendrogram<false>(Z, Z2, N,regions,temp);
    }
